...
use App\Http\Controllers\Api\Landlord\Channels\Meta\Messenger\Weebhook\MessengerWebhookController;
use App\Http\Controllers\Api\Landlord\Channels\Meta\Oauth\FacebookController;
...
use App\Http\Controllers\Api\Landlord\WhatsappController;
...
Route::group(['middleware' => 'guest', 'prefix' => 'auth'], function () {
    ...
    Route::prefix('facebook')->group(function () {
        Route::get('/', [FacebookController::class, 'redirectToProvider'])->middleware('auth:sanctum');
        ...
        Route::post('/webhook', [MessengerWebhookController::class, 'handle']);
    });
...
// ✅ Handle tenant routes for server to server communication (node js)
Route::group(['prefix' => 'whatsapp'], function () {
    Route::post('connect', [WhatsappController::class, 'initialize'])->middleware('auth:sanctum');
    Route::middleware('verifyInternalSecret')
        ->controller(WhatsappController::class)->group(function () {
            Route::post('qr-received', 'qrReceived');
            Route::post('ready', 'ready');
            Route::post('disconnected', 'disconnected');
        });
});
